...
| Delegation Pattern | - 처음에는 각각의 타입들이 서로 강하게 의존하고 있었다. 특히 ChildVC들이 SplitVC를 강하게 알고 있었다 <br> - 이 부분을 올라프에게 피드백을 받은 뒤, SplitVC가 ChildVC를 아는 것은, 알아야만 그 ChildVC를 표현할 수 있으니 당연한 부분이지만, ChildVC들이 SplitVC를 아는 것은 불필요한 내용들을 많이 알게 되는 것 같았다 <br> - 이러한 점에서 이전에 공부했던 Delegation pattern을 떠올렸고, Messenger라는 프로토콜을 구현하여 SplitVC는 이를 채택하게 한 뒤, ChildVC들의 messenger가 자기 자신이라고 알려주는 형태로 구현하게 되었다 <br> - 이후 올라프에게 messenger라는 이름에 대해서 지적을 받은 뒤, messenger가 틀린 말은 아니지만 delegation pattern을 사용했다는 걸 좀 더 분명하게 표현하기 위해서 delegate로 이름을 바꾸게 되었다 |
...
| Theme | description |
| :---: | :--- |
| Memory | - static을 극단적으로 지양하고, 사용되지 않는 요소들에 nil을 할당하여 메모리를 최적화하는 시도를 많이 해보았다 <br> - 하지만 그렇게 함으로써 의도치 않은 버그들이 발생하였고(예를 들면 orientation이 바뀌었을 때 적절한 화면이 나오지 않는 것) 이럴 바에는 그냥 내버려두는 게 나을 거라는 말을 듣게 되었다 <br> - 그럼에도 불구하고 이러한 생각이나 시도는 좋다는 칭찬을 듣기도 했는데... <br> - 아무래도 미련이 많이 남는다. size-class를 고려하여 초기화를 한다거나, size-class에 따라 불러오는 코드를 변경한다거나, 와 같은... |
