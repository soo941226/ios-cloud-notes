...
| Theme | description |
| :---: | :--- |
| Memory | - static을 극단적으로 지양하고, 사용되지 않는 요소들에 nil을 할당하여 메모리를 최적화하는 시도를 많이 해보았다 <br> - 하지만 그렇게 함으로써 의도치 않은 버그들이 발생하였고(예를 들면 orientation이 바뀌었을 때 적절한 화면이 나오지 않는 것) 이럴 바에는 그냥 내버려두는 게 나을 거라는 말을 듣게 되었다 <br> - 그럼에도 불구하고 이러한 생각이나 시도는 좋다는 칭찬을 듣기도 했는데... <br> - 아무래도 미련이 많이 남는다. 이러이러한 것은 더 좋은 방법이 있어서 버그가 없지 않았을까? 와 같은... |
